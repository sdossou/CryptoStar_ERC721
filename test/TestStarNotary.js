...
it('can add the star name and star symbol properly', async() => {
    ...
    let tokenId = 7;
    let instance = await StarNotary.deployed();
    await instance.createStar('CryptoStar', tokenId, {from: accounts[0]});

    //2. Call the name and symbol properties in your Smart Contract and compare with the name and symbol provided
    assert.equal('CryptoStar', await instance.name.call());
    assert.equal('CTS', await instance.symbol.call());
...
it('lets 2 users exchange stars', async() => {
    ...
    let tokenId2 = 9;
    let instance = await StarNotary.deployed();
    await instance.createStar('Cryptostar', tokenId1, {from: accounts[0]});
    await instance.createStar('Cryptostar', tokenId2, {from: accounts[1]});
...
it('lookUptokenIdToStarInfo test', async() => {
    ...
    let tokenId = 11;
    let instance = await StarNotary.deployed();
    await instance.createStar('Cryptostar 11', tokenId, {from: accounts[0]});

    // 2. Call your method lookUptokenIdToStarInfo
    let StarName = await instance.lookUptokenIdToStarInfo(tokenId, {from: accounts[0]});

    // 3. Verify if you Star name is the same
    assert.equal(StarName, 'Cryptostar 11');
});
